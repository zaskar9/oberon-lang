...
Value* LLVMIRBuilder::getOrLoadArrayLength(vector<Value*> &lengths, Value* dopeV, const ArrayTypeNode *type, const uint32_t dim) {
    ...
    if (!lengths[dim]) {
        if (type->isOpen()) {
            ...
            lengths[dim] = builder_.getInt64(type->lengths()[dim]);
        }
    }
    return lengths[dim];
}
...
TypeNode *LLVMIRBuilder::selectors(const NodeReference *ref, TypeNode *base, const SelectorIterator start, const SelectorIterator end) {
    ...
    for (auto it = start; it != end; ++it) {
        ...
        if (sel->getNodeType() == NodeType::parameter) {
            ...
            // Load the dope vector if the array is an open array
            Value *dopeV = nullptr;
            if (array_t->isOpen()) {
                dopeV = valueDopes_[ref->dereference()];
            } else {
            ...
            for (size_t i = 0; i < array->indices().size(); ++i) {
                ...
                for (size_t j = i; j < array_t->dimensions() - 1; ++j) {
                    ...
                    if (j == i) {
                        ...
                        addr = builder_.CreateNUWMul(addr, length);
                    }
                }
                if (offset) {
                    offset = builder_.CreateNSWAdd(offset, addr);
                } else {
                    offset = addr;
                }
            }
...
#if defined(_LLVM_20) || defined(_LLVM_21) || defined(_LLVM_22)
...
void LLVMIRBuilder::visit(ArrayTypeNode &node) {
    uint64_t length = node.lengths()[0];
...
void LLVMIRBuilder::visit(AssignmentNode &node) {
    ...
    if (rType->isArray()) {
        ...
        // TODO Shouldn't the lengths be multiplied here to get the total size?
